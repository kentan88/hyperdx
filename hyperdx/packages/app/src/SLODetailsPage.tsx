  ...
  if (!data || data.length === 0) {
    return (
      <Text c="dimmed" size="sm">
        No significant correlations found or BubbleUp not supported for this SLO.
        Try a larger time window or ensure the SLO was created with the builder.
      </Text>
    );
  }
...
function SLODetailsPage() {
  ...
  const timeStart = useMemo(() => searchedTimeRange[0], [searchedTimeRange[0]?.getTime()]);
  const timeEnd = useMemo(() => searchedTimeRange[1], [searchedTimeRange[1]?.getTime()]);
