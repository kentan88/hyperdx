...
export function injectTimeFilter(query: string, startTime: Date, endTime: Date): string {
...
async function backfillSLOAggregates(slo: ISLO): Promise<void> {
  ...
  if (slo.filter && slo.goodCondition) {
    ...
    logger.warn({ sloId: slo.id }, 'Skipping backfill for Raw SQL SLO (not supported)');
  }
}
...
export async function createSLO(
  ...
  backfillSLOAggregates(newSLO).catch(err => {
    logger.error({ error: err, sloId: newSLO._id }, 'Failed to backfill SLO aggregates');
  });
  ...
  if (updatedSLO && (updates.numeratorQuery || updates.denominatorQuery || updates.filter || updates.sourceTable)) {
    ...
    try {
      ...
      backfillSLOAggregates(updatedSLO).catch(err => {
        logger.error({ error: err, sloId: updatedSLO._id }, 'Failed to re-backfill SLO aggregates');
      });
    } catch (e) {
        ...
        values: [
          {
            ...
            created_at: updatedSLO.createdAt || new Date(),
            updated_at: new Date(),
          },
        ],
        format: 'JSONEachRow',
...
export async function getSLOBubbleUp(
  ...
  try {
    ...
    logger.error({ error: e, sloId }, 'Failed to fetch top attributes for BubbleUp');
  ...
  for (const key of uniqueKeys) {
    ...
    try {
      ...
      logger.error({ error: e, key, sloId }, 'Failed to query BubbleUp attribute');
    }
  }
